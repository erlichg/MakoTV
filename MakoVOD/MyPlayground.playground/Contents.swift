...
let userDir = xbmc.translatePath(Addon.getAddonInfo("profile")!).stringByAddingPercentEscapesUsingEncoding(NSUTF8StringEncoding)
...
func GetSeriesList(catName:String, url:String, var iconimage:String?) {
    ...
    for prm in prms {
        do {
            var name = (prm["title"] as! String).stringByAddingPercentEscapesUsingEncoding(NSUTF8StringEncoding)
            let mode = (prm["url"] as! String).rangeOfString("VOD-") != nil ? 4 : 1
            if (mode == 4 && prm["subtitle"] != nil && prm["subtitle"]!!.count > 0) {
                name = name! + " - " + (prm["subtitle"] as! String).stringByAddingPercentEscapesUsingEncoding(NSUTF8StringEncoding)!
            }
            ...
            var description = ""
            if (prm["brief"]! != nil) {
                description = (prm["brief"] as! String).stringByAddingPercentEscapesUsingEncoding(NSUTF8StringEncoding)!
            }
            if (prm["plot"]! != nil) {
                description += " - " + (prm["plot"] as! String).stringByAddingPercentEscapesUsingEncoding(NSUTF8StringEncoding)!
            }
...
func GetSeasonsList(url:String, iconimage:String) {
    ...
    for prm in prms!["programData"]!["seasons"] as! NSArray{
        do {
            ...
            let name = prm["name"]!!.stringByAddingPercentEscapesUsingEncoding(NSUTF8StringEncoding)
            url = "http://www.mako.co.il" + (prm["url"] as! String)
            let description = prm["brief"]!!.stringByAddingPercentEscapesUsingEncoding(NSUTF8StringEncoding)
            let infos:Dictionary<String, String> = ["Title": name!, "Plot": description!]
            addDir(name!, url: url, mode: 2, iconimage: iconimage, infos: infos)
        } catch {
            print ("\(error)")
        }
    }
    ...
    for prm in prms!["programData"]!["seasons"] as! NSArray {
        ...
        for episode in prm["vods"] as! NSArray{
            do {
                ...
                let name = episode["title"]!!.stringByAddingPercentEscapesUsingEncoding(NSUTF8StringEncoding)! + " - " + episode["shortSubtitle"]!!.stringByAddingPercentEscapesUsingEncoding(NSUTF8StringEncoding)!
                url = "http://www.mako.co.il/VodPlaylist?vcmid="+vcmid!+"&videoChannelId="+videoChannelId!
                let iconimage =  episode["picUrl"] as? String
                let description = episode["subtitle"]!!.stringByAddingPercentEscapesUsingEncoding(NSUTF8StringEncoding)
                let infos:Dictionary<String, String> = ["Title": name, "Plot": description!, "Aired": (episode["date"] as! String).characters.split{$0 == " "}.map(String.init)[1] ]//  [episode["date"].find(" ")+1:]]
...
func PlayItem(url:String) {
    ...
    let videoChannelId=prms!["channelId"] as? String
    var vcmid = prms!["video"]!["guid"] as? String
    url = "http://www.mako.co.il/VodPlaylist?vcmid="+vcmid!+"&videoChannelId="+videoChannelId!
    Play(url)
}
...
func Play(url:String) {
    do {
        ...
        var link = link1
        var text = OpenURL(String(format: link, guid, url.substringFromIndex((end?.startIndex.advancedBy(16))!)))
        var parseError: NSError?
        var parsedObject: AnyObject? = try NSJSONSerialization.JSONObjectWithData(text!,
        ...
        var uuidStr = NSUUID.init().UUIDString
        
        
        var du = "W" + uuidStr.substringToIndex(uuidStr.startIndex.advancedBy(8)) + uuidStr.substringFromIndex(uuidStr.startIndex.advancedBy(9))
        link = link2
        ...
        let listItem = xbmcgui.ListItem(path:final)
        xbmcplugin.setResolvedUrl(true, listitem:listItem)
    } catch {
        print ("\(error)")
    }
}
    ...
    do {
        ...
        for _ in 0...retries {
            req.start { response in
                ...
                let s = NSString(data: response.data, encoding: NSUTF8StringEncoding)
                link = s!.dataUsingEncoding(NSUTF8StringEncoding)
                waiting = false
            }
        }
        while(waiting) {
...
func vod(p:String) -> [Dictionary<String, Any?>] {
...
let u = vod("url=http://www.mako.co.il/mako-vod-index&mode=0&name=MakoTV programs&iconimage=http://img.mako.co.il/2010/08/11/mako%20vod%20c.jpg")[0]["listitem"]
let s = (u as! [String:Any?])["title"] as! String
